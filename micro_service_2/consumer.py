...
def callback(ch, method, properties, body, args):
    ...
    print(f"Service-2: Received in consumer-{args['name']} {body}")


def handle(name):
    channel = initializer()
    print(f'Service-2: Worker-{name} Started consuming messages')
    arg = {'name': name}
    on_message_callback = functools.partial(callback, args=(arg))
    channel.basic_consume('app1', on_message_callback)
